...
//                'only' => ['info', 'list', 'new'],
    ...
    public function actionNew($id)
    {
        ...
        if (!isset($order['service']) || !isset($order['hospital'])) {
            return $this->redirect('/doctor/buy/' . $id);
        }
        ...
        if ($model->save()) {
            ...
            if ($chat = Chat::findOne(['uid' => $id, 'doctor_id' => Yii::$app->user->id])) {
                $r = Common::sendMsg($chat->chat_id, Yii::$app->user->id, $id,
                    '我已经为您生成订单,请<a href="/orders/info/' . $model->order_id . '">查看</a>');
                Yii::warning('chat:' . $r);
            }
            return $this->redirect('/doctor/chat-user/' . $id);
        } else {
            return $this->redirect('/doctor/buy/' . $id);
        }
    }
    ...
    public function actionAccept($id)
    {
        ...
        $data->save();
        $data->push();
        return $this->redirect('/orders/info/' . $id);
    }
